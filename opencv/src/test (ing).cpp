...
#define MA_FILTER       15                  // moving average filter 사이즈
...
std::vector<cv::Point2f> MAF_buf, SEC1_buf, SEC2_buf, SEC3_buf;   // Moving Average Filter 담는 벡터

// Save Video Parameter
const double fps = 30.0;            // 비디오 프레임수
int fourcc = cv::VideoWriter::fourcc('X', '2', '6', '4'); // 비디오 코덱 'M', 'P', '4', 'V'
// cv::VideoWriter video_line("line_test11.mp4", fourcc, fps, cv::Size(800, 600), true);   // 비디오 파일명과 사이즈 등
// cv::VideoWriter video_ori("ori_test11.mp4", fourcc, fps, cv::Size(800, 600), true);   // 비디오 파일명과 사이즈 등
cv::VideoCapture cap("yellow_line_test44.mp4");      
// cv::VideoCapture cap("ori_test7.mp4");     
...
void ImageCallback(const sensor_msgs::Image::ConstPtr &img){
    ...
    try {
        ...
        if(cali_flag == 1){
            ...
            roi2.push_back(cv::Point(img_bgr.cols,SECTIONEND)); // 221
            roi2.push_back(cv::Point(0,SECTIONEND));   //221

            roi3.clear();   // 화면 제일 상단 부분(190~250cm)
            roi3.push_back(cv::Point(0, SECTIONEND)); // 161
            roi3.push_back(cv::Point(img_bgr.cols,SECTIONEND)); // 161
            roi3.push_back(cv::Point(img_bgr.cols,132)); // 221
            roi3.push_back(cv::Point(0,132));   //221
...
void Line_detect(const cv::Mat& img_edge, const cv::Mat& img_draw, bool show_trackbar){
    ...
    if((L1.size()!=0) && (R1.size()!=0)){
        ...
        // if((L1[1].x - L1[0].x) == 0) L1Alpha = 999.9;
        // else L1Alpha = (L1[1].y - L1[0].y)/(float)(L1[1].x - L1[0].x);
        // float L1Beta = L1[1].y - L1Alpha*L1[1].x;
        // float L1p = (SECTION1 - L1Beta)/L1Alpha;
        // if((L1[1].x - L1[0].x) == 0) R1Alpha = 999.9;
        // else R1Alpha = (R1[1].y - R1[0].y)/(float)(R1[1].x - R1[0].x);
        // float R1Beta = R1[1].y - R1Alpha*R1[1].x;
        // float R1p = (SECTION1 - R1Beta)/R1Alpha;
