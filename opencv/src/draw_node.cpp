...
int main(int argc, char **argv)
{
    ...
    pub = it.advertise("/image_raw/streaming", 1);
...
void drawPath(const cv::Mat& img_draw, const cv::Point2f& worldP){
    cv::Mat img = img_draw.clone();
    std::vector<cv::Point2f> path, left, right;
    float alpha = atan2(worldP.x, worldP.y);
    float ld = sqrt((worldP.x*worldP.x)+(worldP.y*worldP.y));
    float radius = fabs(ld/(2.0*sin(alpha)));
    std::cout << "alpha:" << alpha << std::endl;
    // std::cout << "degree:" << alpha*rad2deg << std::endl;
    // std::cout << "ld:" << ld << std::endl;
    std::cout << "radius:" << radius << std::endl;
    if(worldP.x >= 0){
        for(int i=0; i < (2*alpha)*rad2deg*100.; i++){
    ...
    if((leftdrawP.size()!=0) && (rightdrawP.size()!=0)){
        if((topL.y != 0) && (topR.y != 0)){
            cv::line(img, topL, topR, BLUE, 5);
            // std::cout << "topL y:" << topL.y << ", topR y:" << topR.y << std::endl;
        }
        if((topL.y != 0) && (topR.y == 0)){
            float a = (topC.y - topL.y)/(float)(topC.x - topL.x);
            float b = topC.y - a*topC.x; 
            cv::line(img, topL, cv::Point2f{0,b}, BLUE, 5);
            // std::cout << "topL y:" << topL.y << ", topC y:" << topC.y << std::endl;
        }
        if((topL.y == 0) && (topR.y != 0)){
            float a = (topC.y - topR.y)/(float)(topC.x - topR.x);
            float b = topC.y - a*topC.x; 
            cv::line(img, topR, cv::Point2f{0,b}, BLUE, 5);
            // std::cout << "topC y:" << topC.y << ", topR y:" << topR.y << std::endl;
        }
    }
    cv::imshow("draw path", img);
    cv::waitKey(1);
}
...
void drawPath(const cv::Mat& img_draw){
    ...
    if(w > 0){
        ...
        Projection(left, leftdrawP, W2I);
        Projection(right, rightdrawP, W2I);
        //cv::line(img, drawpathP[0], drawpathP[1], BLUE, 5);
        cv::line(img, leftdrawP[0], leftdrawP[1], YELLOW, 5);
        cv::line(img, rightdrawP[0], rightdrawP[1], YELLOW, 5);
    }
    ...
    if(w > 0){
        ...
        //cv::line(img, drawpathP[0], drawpathP[1], BLUE, 5);
        cv::line(img, leftdrawP[0], leftdrawP[1], RED, 5);
        cv::line(img, rightdrawP[0], rightdrawP[1], RED, 5);
    }
    std::vector<cv::Point2f> drawpathP, leftdrawP, rightdrawP;
    cv::Point2f topC, topL, topR;
    cv::Point2f bottomC, bottomL, bottomR;
    ...
    if((leftdrawP.size()!=0) && (rightdrawP.size()!=0)){
        if((topL.y != 0) && (topR.y != 0)){
            cv::line(img, topL, topR, YELLOW, 5);
        }
        if((topL.y != 0) && (topR.y == 0)){
            float a = (topC.y - topL.y)/(float)(topC.x - topL.x);
            float b = topC.y - a*topC.x; 
            ...
            float y = a*x + b;
            cv::line(img, topL, cv::Point2f{x,y}, YELLOW, 5);
        }
        if((topL.y == 0) && (topR.y != 0)){
            ...
            float x = 0;
            float y = a*x + b;
            cv::line(img, topR, cv::Point2f{x,y}, YELLOW, 5);
        }
    }
    ...
    if((leftdrawP2.size()!=0) && (rightdrawP2.size()!=0)){
        ...
        if((topL2.y != 0) && (topR2.y == 0)){
            ...
            float x = (float)img.cols;
            float y = a*x + b;
            cv::line(img, topL2, cv::Point2f{x,y}, RED, 5);
        ...
        if((topL2.y == 0) && (topR2.y != 0)){
            ...
            float x = 0;
            float y = a*x + b;
            cv::line(img, topR2, cv::Point2f{x,y}, RED, 5);
