...
// cv::VideoWriter video_line("/home/annie/line_test513.mp4", fourcc, fps, cv::Size(176, 144), true);   // 비디오 파일명과 사이즈 등
// cv::VideoWriter video_ori("/home/annie/test0513.mp4", fourcc, fps, cv::Size(176, 144), true);   // 비디오 파일명과 사이즈 등
// cv::VideoCapture cap("yellow_line_test44.mp4");      
// cv::VideoCapture cap("/home/annie/test0512.mp4");     
...
void ImageCallback(const sensor_msgs::Image::ConstPtr &img){
    ...
    // cv::imshow("Image section", img_bgr);
    // cv::waitKey(10);
;}
...
void Final_Line(const cv::Mat& img_edge, std::vector<cv::Point2f>& left, std::vector<cv::Point2f>& right, cv::Scalar Lcolor, cv::Scalar Rcolor){
    ...
    // 좌회전 경우
        ...
        // float lc1 = finalLine[0][0] , lc2 = finalLine[0][2] , lc = 0.0;
        // float rc1 = finalLine[1][0] , rc2 = finalLine[1][2] , rc = 0.0;

        // if(fabs(lc1-cx) < fabs(lc2-cx)) lc = lc1;
        // else lc = lc2;

        // if(fabs(rc1-cx) < fabs(rc2-cx)) lc = rc1;
        // else lc = rc2;
    ...
    //     for(int l=0; l<remainR.size(); l++){
    //         if(remainR[l][2]-remainR[l][0] == 0)  slope = 999.9;
    //         else    slope = (remainR[l][3]-remainR[l][1])/
    //                  (float)(remainR[l][2]-remainR[l][0]);

    //         if(slope < lslope){
    //             // 왼쪽 차선 중 기울기가 가장 수직에 가까운 선분 검출
    //             finalLine[0][0] = remainR[l][0];
    //             finalLine[0][1] = remainR[l][1];
    //             finalLine[0][2] = remainR[l][2];
    //             finalLine[0][3] = remainR[l][3];
    ...
    //     for(int r=0; r<remainL.size(); r++){
    //         if(remainL[r][2]-remainL[r][0] == 0)  slope = 999.9;
    //         else    slope = (remainL[r][3]-remainL[r][1])/
    //                  (float)(remainL[r][2]-remainL[r][0]);

    //         if(slope > rslope){
    //             // 오른쪽 차선 중 기울기가 가장 수직에 가까운 선분 검출
    //             finalLine[1][0] = remainL[r][0];
    //             finalLine[1][1] = remainL[r][1];
    //             finalLine[1][2] = remainL[r][2];
    //             finalLine[1][3] = remainL[r][3];
